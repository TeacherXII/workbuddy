...
| **S2** | **S-E09-3** | 表现接线（色盲/时间/雾/模糊/文本/字幕）**+ 恢复后重应用开关** | L | S-E09-2, ③⑤着色器, S-E11-2 | **Given** 开关变更**或 `load_completed`/`checkpoint_restored` 到达**；**When** 应用；**Then** 色盲 `#7A2E2E→#C8862F`+图标、脉动≤2Hz、文本≤150%不破版、字幕渲染、雾/模糊生效；对比度 C-01≥4.5/C-02≥7/C-03≥3。 |
