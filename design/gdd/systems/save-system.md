...
- **UI 对比度**：存档/读档界面正文 ≥4.5:1（**C-01**）、关键指示 ≥7:1（**C-02**）；焦点环 **`#F0C070`** 2px 细描边（vs 面板 `#1B1B1F` = **10.20:1**，过 **C-02**；不靠颜色 alone，美术 §9.4）。
  > **v0.2 更正（Sprint 3 · O-1 Option A，主理人裁决）**：原文为「焦点环 `#C8862F` 细描边（色盲安全，**C-06**）」，两处均误：
  > ① **取值**：`#C8862F` vs `#1B1B1F` 仅 **5.65:1**，达 C-01 但**不达 C-02**。焦点环是「关键操作指示」，须按 C-02（≥7:1）要求 → 改 **`#F0C070`（10.20:1）**。
  > ② **约束归属**：焦点环归 **美术 §9.4「输入可访问性」**，**不归 C-06**。C-06 管的是色盲模式下警报语义色 `#D64545` → `#F0C070` 的替换，与焦点环无关 —— 二者只是恰好共用同一 HEX。原文标「(C-06)」是标注错误。
  > **撞色缓解**（色盲模式下焦点环与警报徽标同为 `#F0C070`）：焦点环 **0Hz 不脉冲 + 矩形描边**，警报徽标 **2.0Hz 双拍 + 实心三角**（脉冲频率与图标面积编码为**常驻**维度，不随色盲开关切换，见 `hud-a11y-signature.md` v1.1 §5.1）→ C-05 三重编码不塌缩。详见 `production/sprints/sprint3-sav-s5-ux-spec.md` §2.3 / FLAG-L / EC-11。
