...
| **暴露 ALERT** | `#D64545`（D7 签字；`#7A2E2E` 仅作低不透明填充 α≤0.35）+ 脉动（≤2Hz，V-02）+ 图标 + 可关屏震（V-03） | `vfx_exposure_alert.tscn` + `ui_icon_alert` | **绝不单色**（C-07）；色盲模式→**`#F0C070`** + 实心三角图标（C-06，art-bible v0.3 §9.1；旧 `#C8862F` 口径已作废——与警戒 CAUTION 同值致亮度比塌缩至 1.00:1）；脉冲 2.0Hz 双拍 + 实心三角**常驻生效**，不随开关切换 |
...
| **行 3** | 色盲三重编码（C-05/C-07） | 锥缘脉动（亮度+形状+位置）、声环（形状+扩散）、暴露（色彩+图标+脉动）；危险语义色 **`#D64545`** 必配图标（C-07）——`#7A2E2E` 仅作 α≤0.35 底纹、**非语义载体**（1.91:1 数学上永不达 C-03）；**图标精灵图集预留槽** | 否（既存精灵+着色器） |
| **行 14** | 输入可达焦点环（**§9.4.1**） | `ui_focus_ring` 用 **`#F0C070` 2px 实线描边 · 恒 0Hz 不脉冲**（10.20:1 过 **C-02**）；键盘/手柄可达；**独立语义槽，非 C-06** | 否（UI 资产） |
...
| --- | --- | --- | --- | --- | --- | --- |
| A-1 | **主成功音**「嗒」 | 存档成功 | `res://arts/audio/ui/sfx_ui_save_success.wav` | 48 kHz / 16-bit / 单声道 / PCM，110 ms | `0.0 dB` | **已制作** |
| A-2 | **主失败音**「顿」 | 存档失败 · 读档失败 | `res://arts/audio/ui/sfx_ui_save_failure.wav` | 48 kHz / 16-bit / 单声道 / PCM，220 ms | `0.0 dB` | **已制作** |
...
> **A-1 / A-2 的「已制作」= 占位音已落地、可加载、真出声**，**不等于**真实音效设计已完成。当前两个文件是合成占位（**PLACEHOLDER**），待真实声音设计替换。交付记录、合成参数与实测自检见 `design/audio/audio-asset-landing.md`。替换时须重跑该文档 §4 的响度自检以复核 **AUD-H1**。
