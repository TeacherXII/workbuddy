...
| 22 | **暴露 ALERT 指示** Exposure Alert | `vfx` + `ui` 图标 | MVP | `#D64545`（D7 签字；`#7A2E2E` 仅作低不透明填充 α≤0.35）+ 脉动（≤2Hz，V-02）+ 图标 + 可关屏震（V-03） | shader 常量上限；HUD 角标 | **绝不单色**（C-07）；色盲模式→**`#F0C070`**+实心三角图标（C-06，art-bible v0.3 §9.1；旧 `#C8862F` 口径已作废——与警戒同值致亮度塌缩 1.00:1）；脉冲 2.0Hz 双拍 + 实心三角**常驻**，不随开关 |
