...
| 22 | **暴露 ALERT 指示** Exposure Alert | `vfx` + `ui` 图标 | MVP | `#D64545`（D7 签字；`#7A2E2E` 仅作低不透明填充 α≤0.35）+ 脉动（≤2Hz，V-02）+ 图标 + 可关屏震（V-03） | shader 常量上限；HUD 角标 | **绝不单色**（C-07）；色盲模式→`#C8862F`高亮+图标（C-06） |
