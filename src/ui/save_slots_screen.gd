...
func _refresh_row(index: int, row: Dictionary, focused: bool) -> void:
	...
	# Corrupt badge: alarm-family stroke through the C-06 resolver, plus the
	# permanent shape channel. Colour is never the only carrier here.
	var badge_label: Label = w["badge"]
	if status == SaveUiModelScript.RowStatus.CORRUPT:
...
func tick(real_delta: float) -> void:
	...
	if before == SaveUiModelScript.State.LOADING_FADE and _fade != null and _fade.visible:
		_set_alpha(_fade, 1.0 - world_fade_alpha())
...
func _apply_corrupt_pulse() -> void:
	...
	for i in range(_row_widgets.size()):
		...
		_set_alpha(badge, lerpf(0.55, 1.0, wave) if corrupt else 1.0)
