...
## 范围（In Scope）
- L2 单例/服务：EventBus、SpatialHashGrid3D、SpatialQueryWrapper(LOS)、LightState 注册、NavServer 封装、SaveManager（检查点）、InputManager、A11ySettings（持久化）。
...
### E01-S5 · 作为（程序员）我要（实现 SaveManager 检查点语义）以便（暴露软失败在最近安全检查点重生，C4）
**Sprint 0**：否（Sprint 1）
**验收**
- Given consistency-review C4「暴露软失败依赖 SaveManager 检查点语义」；概念 Tier1 含存档/读档。
- When 玩家触发 `exposure_detected`（来自 E08）或手动存读。
- Then 世界状态回滚到最近安全检查点（守卫回 RETURN/巡逻、可疑度清零、动态光/物体状态复位），不丢失已解锁进度。
**关联**：patrol-ai §2（GRACE_RT 1.2s→软失败）；consistency-review C4。
...
- Then `InputEvent` 实时（不受 time_scale，ADR-003）；设置写入 `A11ySettings`（色盲/时间缩放/屏震/雾/动模糊/文本/字幕）并持久化（user://）。
...
## 依赖
无（地基）。下游：E02（TimeController）、E03（EventBus/Grid/Input）、E04（LightState）、E05（SpatialQueryWrapper/Grid）、E06（Grid/EventBus）、E07（EventBus）、E08（NavServer/Grid/EventBus）、E09（只读聚合）、E10（CI 基座）。
