...
- **层**：L5 表现/UI 层（架构 §2）
- **依赖**：①–⑦ 全部（只读聚合）；E01（A11ySettings 接口）/ E11（A11ySettings 持久化 SAV-S4）；E02（time_scale_changed）；E08（exposure_detected → 软重开 UI）
- **DAG 优先级**：P4（最后，只读聚合）
...
> **Sprint 2 滑期标注（主理人裁决 1）**：E09 无 Story 滑 S3——a11y 完整包（E09-S5a~S5d + E09-S7）**全留 Sprint 2**。注意：手动存档/读档 UI 不在此 Epic，归 **E11 · SAV-S5**（滑 S3，裁决 1），详情见 `production/epics/E11-save-manager.md`。
...
**Sprint 2**：是（实现；拆 S5a~S5d 见下）
**验收**
- Given core-hud-a11y §7 + control-manifest：C-05/C-06/C-07（色盲 `#D64545`→`#F0C070`+**实心三角**图标）、T-01/T-02（时间 0.1–1.0 默认 0.25）、V-03（屏震默认关）、V-04（雾选项）、V-05（动模糊默认关）、X-01（文本 100–150%）、X-02（字幕说话者+图标）、V-01（禁>3Hz 频闪）、V-02（脉动≤2Hz）、V-06（转场 ease）。
- When 玩家在设置菜单调整。
- Then 写入 `A11ySettings`（经 E11 SaveManager 持久化）；生效于对应系统（色盲模式→E05/E08 图标；时间滑杆→E02；屏震/雾/动模糊→VFX；文本/字幕→UI）。
**关联**：core-hud-a11y §7；control-manifest C-/V-/X-/T- 全约束；E01-S6（A11ySettings）；E02-S7（时间滑杆）。

#### E09-S5a · 色盲模式完整枚举 + 危险色映射 + 图标（C-05/C-06/C-07）
**Sprint 2**：是
**验收**
- Given 色盲模式须为完整枚举（OFF/PROTAN/DEUTAN/TRITAN）；警报语义色 `#D64545` → **`#F0C070`** + **实心三角 `!`** 图标（C-06）；绝不单色（C-07）。
- When 玩家切换色盲模式。
- Then 危险色映射生效于 HUD/世界要素；图标三重编码（亮度+形状+图标）。**脉冲频率（警戒 0.5Hz 单拍 / 警报 2.0Hz 双拍）与图标面积编码（空心圆环 / 实心三角）常驻生效，不随本开关切换。**
**关联**：core-hud-a11y §7；control-manifest C-05/C-06/C-07；E05/E08（图标渲染）。

> **更正说明（S3-B 残留收口 · C-06 现行口径 · 林绘澄签字）**：本 Epic 原 L58 / L66 / L100 三处写的映射 **`#7A2E2E` → `#C8862F`（旧 C-06）已作废**。根因：警戒 CAUTION **本就是** `#C8862F`，该映射令警戒与警报**亮度比塌缩至 1.00:1**，C-05 三重编码中的「亮度」维度彻底失效 —— 等于**在无障碍验收条款内部犯 C-05**。**现行口径 = `#D64545` → `#F0C070`**（vs 警戒 `#C8862F` = **1.81:1**；vs 面板 `#1B1B1F` = **10.20:1**，过 C-02）。依据 `design/art/art-bible.md` **v0.4** §9.1 / `design/art/hud-a11y-signature.md` **v1.1** §4.1·§5.2 / `design/art/accessibility-matrix.md` 行 4·§3.1。
> **工程侧对应**：源码常量 **`HUD_COLOR_ALARM_CB` 已退休**，由 **`HUD_COLOR_DANGER_CB`**（`#F0C070`）取代（S3-B 落地）；验收断言须引新常量名，勿再引旧名。
> **勿全局替换 `#C8862F`**：该色作为**警戒 CAUTION** 与**暖光/光池/落足微光**的取值**完全现行有效**，本次只改「色盲档警报映射」这一个槽位。

#### E09-S5b · 时间缩放滑杆 [0.1,1.0] 默认 0.25（T-01/T-02）
**Sprint 2**：是
**验收**
- Given `time_scale_user ∈ [0.1, 1.0]`，默认 0.25；须驱动 `TimeController`（E02）。
- When 玩家拖动时间滑杆。
- Then `Engine.time_scale` 经 `time_scale_changed` 设到目标值；玩家输入实时不受缩放（ADR-003）。
**关联**：core-hud-a11y §7；control-manifest T-01/T-02/V-06；E02（TimeController）。

#### E09-S5c · 眩晕控制：屏震关（V-03）/ 雾三态（V-04）/ 动模糊关（V-05）
**Sprint 2**：是
**验收**
- Given `screen_shake=false` 默认关；`fog_option{FULL,REDUCED,OFF}` 三态（现 `fog_enabled:bool` 仅两态须扩）；`motion_blur=false` 默认关。
- When 玩家切换。
- Then 屏震/雾/动模糊按设置生效于 VFX 层；禁 >3Hz 频闪（V-01）。
**关联**：core-hud-a11y §7；control-manifest V-01/V-03/V-04/V-05；VFX 层（E03/E04）。

#### E09-S5d · 文本缩放 [1.0,1.5]（X-01）+ 字幕说话者+图标（X-02）
**Sprint 2**：是
**验收**
- Given `text_scale ∈ [1.0, 1.5]`（现固定 1.0）；`subtitles=true` 说话者+图标。
- When 玩家调整文本缩放 / 开关字幕。
- Then UI 文本按 `text_scale` 缩放；字幕显示说话者+图标。
**关联**：core-hud-a11y §7；control-manifest X-01/X-02；E09-S6（字幕载体）。
...
- Then HUD 显示暴露 ALERT：`#D64545`（D7 签字；`#7A2E2E` 仅作低不透明填充 α≤0.35）+图标+脉动≤2Hz；色盲模式→**`#F0C070`** 高亮+**实心三角**图标（C-06）；屏震默认关（V-03）；触发软重开 UI（经 E11 SaveManager 检查点，C4）。
**关联**：core-hud-a11y §5；patrol-ai §2；consistency-review C4；control-manifest C-06/C-07/V-02/V-03。

### E09-S7 · 作为（系统）我要（A11ySettings 数据模型扩展 + 落盘/读盘接 SaveManager）以便（设置跨会话保留）
**Sprint 2**：是
**验收**
...
- When 扩展为完整 Tier2 模型并经 SaveManager 落盘。
- Then 新增 `colorblind_mode` 枚举（OFF/PROTAN/DEUTAN/TRITAN）、`time_scale_user:float[0.1,1.0]`、`fog_option:int{FULL,REDUCED,OFF}`、`subtitles:bool`；经 E11 SAV-S4 落盘/读盘；缺字段向后兼容默认。
**关联**：core-hud-a11y §7；a11y_settings.gd（Sprint 0 切片）；E11 SAV-S1/S4；control-manifest（数据契约）。

## 依赖
①–⑦ 全部（只读聚合，不反向驱动玩法）；E01（A11ySettings 接口）/ E11（A11ySettings 持久化 SAV-S4）；E02（time_scale_changed）；E08（exposure_detected）。被依赖：无（终端呈现层）。
