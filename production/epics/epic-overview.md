...
> 粒度 = 每个核心系统 1 个 Epic（E02–E09）+ 1 个工程基座/管线 Epic（E01）+ 1 个发布/CI Epic（E10）。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **E01** | 工程基座与管线（L2 基础设施 + 资产管线 + CI 基座） | L2 基础设施（EventBus / SpatialHashGrid3D / SpatialQueryWrapper / LightState / NavServer / SaveManager / InputManager / A11ySettings）+ 资产导入管线（asset-manifest §1.3） | L2 / L1 | —（地基） | P0 地基 | **Must** | L |
...
- **M**：E02 / E03 / E04 / E05 / E07 / E09 / E10 — 单系统核心逻辑明确。
...
```
[E01 工程基座/L2] ── EventBus · Grid · SpatialQueryWrapper(LOS) · TimeController · LightState · NavServer · SaveManager · InputManager · A11ySettings
   │
...
### Sprint 2 — 深化 / 可访问性 / 发布
- **包含**：E07 完整（DECOY/LIGHT_TOGGLE/TRAP/SMOKE 四类 + charges 限量 + entity-inventory 配置）/ E08 变体（循声猎犬 / 暗视哨兵，参数覆盖不新增系统）/ E09 a11y 完整包（色盲模式 C-05/C-06/C-07 + 时间滑杆 T-01/T-02 + 屏震关 V-03 + 雾选项 V-04 + 动模糊关 V-05 + 文本缩放 X-01 + 字幕 X-02）/ E10 完整（Steam 导出 + 预算校验 harness + draw call ≤400 实测）。
- **退出标准**：Tier2 守卫 16 同行不破动态光 32（R-02，光 LOD ADR-004）；可访问性开关全部生效；Steam 构建产出。
